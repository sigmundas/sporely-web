-- Fix preparation-level filtering in get_public_spore_comparison_set.
--
-- Bug fixed:
--   The previous implementation checked only the LATEST microscope image per
--   observation when applying sample_type / mount_reagent / contrast_method
--   filters. An observation with multiple images (e.g. fresh + spore_print)
--   would be excluded when filtering by the prep type of the non-latest image,
--   and when included, the measurement counts were taken from ALL images
--   regardless of prep type.
--
-- Changes:
--   taxon_obs:     Replace JOIN LATERAL … LIMIT 1 / AND latest_img.* pattern
--                  with EXISTS subqueries. An observation qualifies when it has
--                  ANY matching-preparation microscope image.
--   spore_eligible: Add CROSS JOIN norm n and per-image prep filters so only
--                  measurements from matching-prep images are counted.
--   raw_meas:      Same per-image prep filters so aggregate stats are
--                  prep-scoped.
--   obs_means:     Same per-image prep filters in both lateral joins, plus
--                  carry the active filter values through to the output row.
--   observations:  Populate sampleType / mountReagent / contrastMethod in each
--                  per-observation jsonb object from the carried filter values.
...
AS $$
  ...
  raw_meas AS (
    ...
    JOIN public.observation_images i
      ...
      AND (se.filter_sample_type     IS NULL OR lower(coalesce(i.sample_type,  '')) = se.filter_sample_type)
      AND (se.filter_mount_reagent   IS NULL OR lower(coalesce(i.mount_medium, '')) = se.filter_mount_reagent)
      AND (se.filter_contrast_method IS NULL OR lower(coalesce(i.contrast,     '')) = se.filter_contrast_method)
    JOIN public.spore_measurements m ON m.image_id = i.id
      AND m.length_um IS NOT NULL
      AND (
        m.measurement_type IS NULL
  ...
  -- matching-prep images. Filter values are carried through for output.
  obs_means AS (
    SELECT
    ...
    coalesce((
      SELECT jsonb_agg(
        jsonb_strip_nulls(jsonb_build_object(
          ...
          'sampleType',         om.filter_sample_type,
          'mountReagent',       om.filter_mount_reagent,
          'contrastMethod',     om.filter_contrast_method,
          'sporeN',             om.spore_n,
