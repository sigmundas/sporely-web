-- Public spore comparison set with optional taxon (species or genus) and filter parameters.
--
-- Returns one row for the matched taxon + filter combination containing:
--   taxon metadata, filtered coverage metrics, aggregate spore statistics
...
--   - Microscopy image filters (sample_type, mount_reagent, contrast_method)
--     applied against the latest microscope image for each observation.
...
AS $$
  ...
  taxon_obs AS (
    ...
    -- Latest microscope image for this observation (for image-level filters).
    JOIN LATERAL (
      SELECT
        i.id,
        i.sample_type,
    ...
    WHERE o.visibility = 'public'::text
      ...
      -- Optional microscopy image filters (case-insensitive, against latest image).
      AND (n.sample_type     IS NULL OR lower(coalesce(latest_img.sample_type,  '')) = n.sample_type)
      AND (n.mount_reagent   IS NULL OR lower(coalesce(latest_img.mount_medium, '')) = n.mount_reagent)
      AND (n.contrast_method IS NULL OR lower(coalesce(latest_img.contrast,     '')) = n.contrast_method)
  ),
  ...
  -- Per-observation means computed fresh from raw measurements.
  obs_means AS (
    SELECT
