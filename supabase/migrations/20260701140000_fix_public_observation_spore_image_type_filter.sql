-- Fix: sporeMeasurementCount and sporePoints excluded microscope images
-- uploaded before image_type was a required field (those images have
-- image_type IS NULL in Supabase).  Relax the filter to:
--   (i.image_type IS NULL OR i.image_type = 'microscope')
--
-- Safety: field-image measurements carry measurement_type = 'field' which
-- is already excluded by the measurement_type allowlist, so relaxing the
-- image_type gate does not expose non-spore measurements.
...
AS $$
  ...
  enriched AS (
    ...
    JOIN public.observations o
      ON o.id = c.id
    LEFT JOIN LATERAL (
      -- Latest microscope image for contrast/mount/sample metadata.
      SELECT
        i.id,
        i.contrast,
        i.mount_medium,
    ...
    LEFT JOIN LATERAL (
      -- Count spore-type measurements from microscope images.
    ...
    LEFT JOIN LATERAL (
      -- Individual spore measurement points (L, W, Q, cropUrl).
