-- Fix preparation-level filtering in search_public_observations.
--
-- Bug fixed:
--   The previous implementation filtered by contrast/mount/sample via the
--   LATEST microscopy image lateral join. An observation with multiple images
--   (e.g. fresh + spore_print) would be excluded when filtering by the prep
--   type of the non-latest image.
--
-- Changes:
--   The WHERE conditions in the final SELECT that previously compared against
--   latest_image.contrast, latest_image.mount_medium, and latest_image.sample_type
--   are replaced with EXISTS subqueries that check ALL non-deleted/purged
--   microscope images — matching the semantics of get_public_spore_comparison_set.
--
--   The lateral join that provides latest_image is kept unchanged because the
--   output row fields contrastMethod, mountReagent, sampleType, and hasMicroscopy
--   still read from it.
...
AS $$
  ...
  WHERE (n.has_microscopy IS NULL OR e.has_microscopy = n.has_microscopy)
    ...
    -- Prep filters via EXISTS on ALL microscope images (not just the latest),
    -- matching get_public_spore_comparison_set semantics.
    AND (n.contrast IS NULL OR EXISTS (
      ...
      WHERE i2.observation_id = e.id
        ...
        AND lower(coalesce(i2.contrast, '')) = lower(n.contrast)
    ))
    AND (n.mount IS NULL OR EXISTS (
      ...
      WHERE i2.observation_id = e.id
        AND i2.deleted_at IS NULL AND i2.purged_at IS NULL
        AND i2.image_type = 'microscope'
        AND lower(coalesce(i2.mount_medium, '')) = lower(n.mount)
    ))
    AND (n.sample IS NULL OR EXISTS (
      ...
      WHERE i2.observation_id = e.id
        AND i2.deleted_at IS NULL AND i2.purged_at IS NULL
        AND i2.image_type = 'microscope'
        AND lower(coalesce(i2.sample_type, '')) = lower(n.sample)
    ))
