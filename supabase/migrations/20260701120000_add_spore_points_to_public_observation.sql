-- Expose individual spore measurement points through get_public_observation.
--
-- Points are returned as a JSONB array in "sporePoints", gated by the same
-- spore_data_visibility = 'public' check that guards sporeSummary.
-- Only points with both length_um and width_um populated are included.
-- crop URLs are formed from thumb_key using the standard media CDN prefix.
-- q = width_um / length_um, rounded to 4 decimal places.
    ...
    LEFT JOIN LATERAL (
      SELECT jsonb_agg(
        jsonb_build_object(
          ...
          'widthUm',  m.width_um,
          'q',        round((m.width_um / nullif(m.length_um, 0))::numeric, 4)::double precision,
          'cropUrl',  CASE
