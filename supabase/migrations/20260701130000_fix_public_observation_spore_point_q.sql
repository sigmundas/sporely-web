-- Fix sporePoints JSON in get_public_observation:
--   1. q = length_um / width_um  (was width_um / length_um — the convention is Q = L/W)
--   2. Add observationId and imageId to every point object
--
-- RETURNS TABLE signature is unchanged; uses CREATE OR REPLACE.
    ...
    LEFT JOIN LATERAL (
      SELECT jsonb_agg(
        jsonb_build_object(
          ...
          'widthUm',        m.width_um,
          -- Q = L / W (length-to-width ratio; normally > 1 for elongated spores)
