-- Taxonomy-v3 additive core: sparse identity registry, historical snapshots,
-- current resolution links, release audit tables, and trusted installer.
-- This migration intentionally leaves every legacy taxonomy object untouched.
--
...
comment on column reconciliation_manifest_audit.input_file_sha256 is
  'SHA-256 of the raw manifest file bytes as loaded from disk. Distinct from manifest_semantic_sha256 which excludes non-semantic fields; two different input files with the same semantic content produce the same manifest_semantic_sha256 but different input_file_sha256.';

-- IF NOT EXISTS makes a byte-identical retry harmless, but must not mask a
-- pre-existing object with an incompatible contract. Check the identity,
-- linkage, and audit columns before any installer function is installed.
do $$
declare
  v_expected record;
  v_actual_type text;
...
begin
  for v_expected in
    select * from (values
      ('registry_concept', 'sporely_taxon_id', 'integer', true),
      ('registry_concept', 'scope_state', 'text', false),
      ('registry_concept', 'cache_state', 'text', true),
      ('external_mapping', 'external_id', 'text', true),
      ('external_mapping', 'sporely_taxon_id', 'integer', true),
      ('identification_snapshot', 'observation_id', 'text', true),
      ('identification_snapshot', 'original_signals', 'jsonb', true),
      ('resolution_link', 'observation_id', 'text', true),
      ('resolution_link', 'resolved_sporely_taxon_id', 'integer', false),
      ('release_installation', 'release_id', 'text', true),
      ('supplement_installation', 'supplement_release_id', 'text', true),
      ('reconciliation_manifest_audit', 'manifest_semantic_sha256', 'text', true),
      ('reconciliation_manifest_audit', 'input_file_sha256', 'text', false),
      ('reconciliation_manifest_audit', 'record_count', 'integer', true)
    ) as expected(table_name, column_name, data_type, is_not_null)
  loop
    select format_type(a.atttypid, a.atttypmod), a.attnotnull
      into v_actual_type, v_actual_not_null
      from pg_attribute a
     where a.attrelid = format('taxonomy_v3.%I', v_expected.table_name)::regclass
       and a.attname = v_expected.column_name
       and a.attnum > 0
       and not a.attisdropped;

    if v_actual_type is distinct from v_expected.data_type
       or v_actual_not_null is distinct from v_expected.is_not_null then
      raise exception
        'incompatible taxonomy_v3 object: %.% expected type=% not_null=%, found type=% not_null=%',
        v_expected.table_name, v_expected.column_name, v_expected.data_type,
        v_expected.is_not_null, v_actual_type, v_actual_not_null;
    end if;
  end loop;

  if not exists (
    select 1 from pg_constraint
     where conrelid = 'taxonomy_v3.external_mapping'::regclass
       and contype = 'u'
       and pg_get_constraintdef(oid) = 'UNIQUE (source_system, namespace, external_id)'
  ) then
    raise exception 'incompatible taxonomy_v3.external_mapping: missing namespaced identifier uniqueness constraint';
  end if;
end $$;
...
begin
  ...
  for v_supp in select e.value from jsonb_array_elements(coalesce(p_supplements,'[]'::jsonb)) e(value) loop
    ...
    for v_map in select e.value from jsonb_array_elements(coalesce(v_supp->'external_mappings','[]'::jsonb)) e(value) loop
      insert into registry_concept (
        ...
        -- NULL metadata from a sparse supplement may be enriched later.
        -- Scope/cache are membership facts, not identity; preserve the first
        -- materialized values instead of treating another observation's
        -- scope/cache context as an identity conflict.
        set canonical_name = coalesce(registry_concept.canonical_name, excluded.canonical_name),
            rank           = coalesce(registry_concept.rank, excluded.rank),
            scope_state    = coalesce(registry_concept.scope_state, excluded.scope_state)
        where (registry_concept.canonical_name is null or excluded.canonical_name is null or registry_concept.canonical_name = excluded.canonical_name)
          and (registry_concept.rank           is null or excluded.rank           is null or registry_concept.rank           = excluded.rank);
      get diagnostics v_row_count = row_count;
      if v_row_count = 0 then
        raise exception 'W3-A2 registry_concept identity conflict: sporely_taxon_id=% already anchored with different canonical identity',
  ...
  -- W3-A2: reject same semantic-SHA reinstall whose input file bytes,
  -- record_count, or state totals differ from what was recorded previously.
  insert into reconciliation_manifest_audit (
    manifest_semantic_sha256, input_file_sha256, record_count, state_counts
  ) values (
    v_manifest_sha,
    p_reconciliation_manifest->>'input_file_sha256',
    v_record_count,
    coalesce(p_reconciliation_manifest->'aggregate_counts', p_reconciliation_manifest->'state_counts', '{}'::jsonb)
  ) on conflict (manifest_semantic_sha256) do update
    set applied_at = excluded.applied_at
    where reconciliation_manifest_audit.record_count      = excluded.record_count
      and reconciliation_manifest_audit.state_counts      = excluded.state_counts
      and reconciliation_manifest_audit.input_file_sha256 is not distinct from excluded.input_file_sha256;
  get diagnostics v_row_count = row_count;
  if v_row_count = 0 then
    raise exception 'W3-A2 manifest sha reuse with different bytes/counts/state_totals for semantic sha %', v_manifest_sha
      using errcode = 'unique_violation';
  end if;
  ...
  for v_record in select e.value from jsonb_array_elements(coalesce(p_reconciliation_manifest->'records','[]'::jsonb)) e(value) loop
    ...
    if v_resolved is not null then
      insert into registry_concept (
        ...
        -- Permit sparse NULL-to-known metadata enrichment. Scope/cache remain
        -- the registry concept's existing membership facts and are not part
        -- of the canonical-name/rank identity conflict check.
        set canonical_name = coalesce(registry_concept.canonical_name, excluded.canonical_name),
            rank           = coalesce(registry_concept.rank, excluded.rank),
            scope_state    = coalesce(registry_concept.scope_state, excluded.scope_state)
        where (registry_concept.canonical_name is null or excluded.canonical_name is null or registry_concept.canonical_name = excluded.canonical_name)
      ...
      if v_row_count = 0 then
        raise exception 'W3-A2 registry_concept identity conflict: sporely_taxon_id=% already anchored with different canonical identity',
          coalesce((v_map->>'sporely_taxon_id')::integer, v_resolved) using errcode = 'unique_violation';
      end if;
      v_reg_added := v_reg_added + v_row_count;
