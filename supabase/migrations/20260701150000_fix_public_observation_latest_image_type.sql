-- Fix: latest_image lateral still required i.image_type = 'microscope', causing
-- hasMicroscopy=false and null contrast/mount/sample for legacy observations
-- whose microscope images have image_type IS NULL.
--
-- The safer condition adds NULL-typed images that demonstrably carry spore
-- measurements (manual/spore/spores), so untagged field images are not
-- mistakenly promoted to microscopy.
...
AS $$
  ...
  enriched AS (
    ...
    JOIN public.observations o
      ON o.id = c.id
    LEFT JOIN LATERAL (
      -- Latest microscopy-style image for contrast/mount/sample metadata.
      -- Include explicitly-tagged microscope images plus untagged images that
      -- demonstrably carry spore measurements (safe proxy for legacy uploads).
      -- Untagged images with no spore measurements are left out so that old
      -- field photos without a tag do not get treated as microscopy.
      SELECT
        i.id,
        i.contrast,
        i.mount_medium,
    ...
    LEFT JOIN LATERAL (
      -- Count spore-type measurements.
      -- Include images with NULL image_type (uploaded before tagging was enforced).
    ...
    LEFT JOIN LATERAL (
      -- Individual spore points.
      -- Include images with NULL image_type to match sporeMeasurementCount.
      SELECT jsonb_agg(
        jsonb_build_object(
          'id',             m.id::text,
