...
-- Applied only against a disposable local Supabase stack by
-- scripts/taxonomy-v3/install-release-chain.mjs when its --observations
-- flag is set.
--
...
begin
  if new.resolved_sporely_taxon_id is distinct from old.resolved_sporely_taxon_id then
    if current_setting('request.jwt.claim.role', true) is distinct from 'service_role'
      ...
      raise exception 'public.observations.resolved_sporely_taxon_id can only be updated by service_role';
    end if;
  end if;
  return new;
end $$;
